...
def observations_to_image(observation: Dict, info: Dict) -> np.ndarray:
    ...
    if "normal" in observation:
        observation_size = observation["normal"].shape[0]
        normal = observation["normal"]
        if not isinstance(normal, np.ndarray):
            normal = normal.cpu().numpy().astype(np.uint8)
        egocentric_view.append(normal)
    if "keypoints3d" in observation:
        observation_size = observation["keypoints3d"].shape[0]
        keypoints3d = observation["keypoints3d"]
        if not isinstance(keypoints3d, np.ndarray):
            keypoints3d = keypoints3d.cpu().numpy().astype(np.uint8)
        keypoints3d = np.dstack((keypoints3d, keypoints3d, keypoints3d))
        egocentric_view.append(keypoints3d)
