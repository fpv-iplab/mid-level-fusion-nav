...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, observations):
        ...
        if self._n_input_rgb > 0:
            rgb_observations = observations["rgb"] # size 
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            rgb_observations = rgb_observations.permute(0, 3, 1, 2)
            if rgb_observations.max() > 1.: # not scaled
                rgb_observations = rgb_observations / 255.0  # scale RGB
        ...
        x = torch.cat(cnn_input, dim=1) # tensor [BATCH x CHANNEL x HEIGHT X WIDTH]
        x = F.avg_pool2d(x, 2)
